    ...
    if (lookupHandle(path, rpcStatus, res)) {
        ...
        if (res.diropres_u.diropres.attributes.type == NFLNK) {
            ...
            if (rpcStatus == RPC_SUCCESS && readLinkRes.status == NFS_OK) {
                const QString linkDest = QFile::decodeName(readLinkRes.readlinkres_u.data);
                QString linkPath;
...
void NFSProtocolV2::listDir(const KUrl& url)
{
    ...
    if (isExportedDir(path)) {
        kDebug(7121) << "Listing exported dir";
        ...
        for (QStringList::const_iterator it = virtualList.constBegin(); it != virtualList.constEnd(); ++it) {
            kDebug(7121) << (*it) << "found in exported dir";
...
void NFSProtocolV2::stat(const KUrl& url)
{
    kDebug(7121) << url;
    ...
    if (attrAndStat.attrstat_u.attributes.type == NFLNK) {
        ...
        if (readLink(path, rpcStatus, readLinkRes, nameBuf)) {
            linkDest = QFile::decodeName(readLinkRes.readlinkres_u.data);
        } else {
            entry.insert(KIO::UDSEntry::UDS_LINK_DEST, i18n("Unknown target"));
...
void NFSProtocolV2::mkdir(const KUrl& url, int permissions)
{
    kDebug(7121) << url;
...
void NFSProtocolV2::del(const KUrl& url, bool)
{
    int rpcStatus;
    nfsstat nfsStatus;
    if (!remove(url.path(), rpcStatus, nfsStatus)) {
        ...
        kDebug(7121) << "Could not delete" << url;
        return;
    }
...
void NFSProtocolV2::chmod(const KUrl& url, int permissions)
{
    kDebug(7121) << url;
...
void NFSProtocolV2::get(const KUrl& url)
{
    kDebug(7121) << url;
...
void NFSProtocolV2::put(const KUrl& url, int _mode, KIO::JobFlags flags)
{
    kDebug(7121) << url << _mode;
...
void NFSProtocolV2::rename(const KUrl& src, const KUrl& dest, KIO::JobFlags _flags)
{
...
void NFSProtocolV2::copySame(const KUrl& src, const KUrl& dest, int _mode, KIO::JobFlags _flags)
{
    kDebug(7121) << src << "to" << dest;
    ...
    if (srcFH.isLink()) {
        ...
        const QString linkPath = QFile::decodeName(readLinkRes.readlinkres_u.data);
    ...
    if (error) {
        ...
        if (getAttr(srcPath, rpcStatus, attrRes)) {
            ...
            if (!setAttr(destPath, attributes, rpcStatus, attrSetRes)) {
                kDebug(7121) << "Failed to restore mtime, ignoring..." << rpcStatus << attrSetRes;
            }
        }
...
void NFSProtocolV2::copyFrom(const KUrl& src, const KUrl& dest, int _mode, KIO::JobFlags _flags)
{
    kDebug(7121) << src << "to" << dest;
    ...
    if (srcFH.isLink()) {
        ...
        QFile::link(QFile::decodeName(readLinkRes.readlinkres_u.data), destPath);
    ...
    do {
        ...
        if (readArgs.offset == 0) {
            ...
            KMimeType::Ptr p_mimeType = KMimeType::findByNameAndContent(src.fileName(), QByteArray::fromRawData(readRes.readres_u.reply.data.data_val, bytesRead));
            m_slave->mimeType(p_mimeType->name());
        }
    ...
    if (error) {
        ...
        if (!mtimeStr.isEmpty()) {
            ...
            if (dt.isValid()) {
                kDebug(7121) << "Setting modification time to" << dt.toTime_t();
...
void NFSProtocolV2::copyTo(const KUrl& src, const KUrl& dest, int _mode, KIO::JobFlags _flags)
{
    kDebug(7121) << src << "to" << dest;
...
void NFSProtocolV2::symlink(const QString& target, const KUrl& dest, KIO::JobFlags flags)
{
    const QString destPath(dest.path());
    if (isExportedDir(QFileInfo(destPath).path())) {
        m_slave->error(KIO::ERR_ACCESS_DENIED, destPath);
